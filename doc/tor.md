...
In a typical situation, this suffices to run behind a Tor proxy:
	...
	./globedexd -proxy=127.0.0.1:9050
...
versions of Tor see [Section 3](#3-automatically-listen-on-tor).*
	...
	HiddenServiceDir /var/lib/tor/globedex-service/
	HiddenServiceVersion 2
	HiddenServicePort 42226 127.0.0.1:42226
	HiddenServicePort 61472 127.0.0.1:61472

The directory can be different of course, but (both) port numbers should be equal to
your globedexd's P2P listen port (42226 by default).

	-externalip=X   You can tell globedex about its publicly reachable address using
	                ...
	                /var/lib/tor/globedex-service/hostname. For connections
...
In a typical situation, where you're only reachable via Tor, this should suffice:
	...
	./globedexd -proxy=127.0.0.1:9050 -externalip=globedexzj6l4cvo2fxy.onion -listen

(obviously, replace the .onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

	./globedexd ... -bind=127.0.0.1
...
and open port 42226 on your firewall (or use -upnp).

If you only want to use Tor to reach .onion addresses, but not use it as a proxy
for normal IPv4/IPv6 communication, use:

	./globedexd -onion=127.0.0.1:9050 -externalip=globedexzj6l4cvo2fxy.onion -discover
...
in `torrc` config file. For cookie authentication the user running globedexd must have read
access to the `CookieAuthFile` specified in Tor configuration. In some cases this is
preconfigured and the creation of a hidden service is automatic. If permission problems
are seen with `-debug=tor` they can be resolved by adding both the user running Tor and
the user running globedexd to the same group and setting permissions appropriately. On
Debian-based systems the user running globedexd can be added to the debian-tor group,
