...
The build script we'll be using is contained in the PIVX github repository ([contrib/gitian-build.py](https://github.com/globedex-project/globedex/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the PIVX repository and will need to fetch this script with the following commands:
...
- Clone the gitian-builder, gitian.sigs, globedex-detached-sigs, and globedex GitHub repos.
