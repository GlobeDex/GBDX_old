...
class FakeStakeTest(GlobeDexTestFramework):
    ...
    def fake_stake(self,
        ...
        for i in range(3):
            ...
            stakeInputs = self.get_prevouts(1, staking_utxo_list, nHeight - 1)
            # Update stake inputs for second block sent on forked chain (must stake the same input)
            if not isMainChain and i == 1:
                stakeInputs = self.get_prevouts(1, [stakedUtxo], nHeight-1)
