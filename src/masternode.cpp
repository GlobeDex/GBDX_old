...
CAmount CMasternode::GetMasternodeNodeCollateral(int nHeight) 
{
    ...
    if(nHeight > 450000) return 4000 * COIN;
    if(nHeight > 400000) return  3000 * COIN;
    if(nHeight > 250000) return  2000 * COIN;
    if(nHeight > 200000) return  1000 * COIN;
    if(nHeight > 175000) return  900 * COIN;
    if(nHeight > 150000) return  800 * COIN;
    if(nHeight > 130000) return  500 * COIN;
    if(nHeight > 100000) return   200 * COIN;  
    if(nHeight >    30) return     100 * COIN;
...
{ 
    ...
    if(nHeight > 10000000) return    0 * COIN; 
    if(nHeight >       1) return      0.01 * COIN;
    if(nHeight >       0) return 10000000 * COIN;
...
{
    if(nHeight > 10000000) return GetBlockValue(nHeight) * 80 / 100;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 GBDX tx got MASTERNODE_MIN_CONFIRMATIONS
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 GBDX tx -> 1 confirmation
