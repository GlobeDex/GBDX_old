...
#include "qt/globedex/defaultdialog.h"
#include "qt/globedex/settings/settingsfaqwidget.h"
...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("GlobeDex client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("GlobeDex"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("GlobeDex"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
