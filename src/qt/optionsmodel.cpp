...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeGlobeDexAmount") || reset)
        SoftSetArg("-anonymizeglobedexamount", settings.value("nAnonymizeGlobeDexAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in globedex.cpp during shotdown to also remove the window positions
