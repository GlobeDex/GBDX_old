...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter GBDX address (e.g. %1)").arg("GQeVBSRsUWWHQnquWKJSvZPttbXrWQRtoT"));
...
{
    // return if URI is not valid or is no GBDX: URI
...
{
    // Convert globedex:// to globedex:
    //
    //    Cannot handle this later, because globedex:// will cause Qt to see the part after // as host,
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "globedex.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for globedex.lnk
...
{
    return GetAutostartDir() / "globedex.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a globedex.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=GlobeDex\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the globedex app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add globedex app to startup item list
