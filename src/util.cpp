...
const char * const PIVX_CONF_FILENAME = "globedex.conf";
const char * const PIVX_PID_FILENAME = "globedex.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";
const char * const PIVX_ACTIVE_MASTERNODE_CONF_FILENAME = "activemasternode.conf";


// GlobeDex only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\globedex
// Windows >= Vista: C:\Users\Username\AppData\Roaming\globedex
// Mac: ~/Library/Application Support/globedex
// Unix: ~/.globedex
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GlobeDex";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty globedex.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override globedex.conf
